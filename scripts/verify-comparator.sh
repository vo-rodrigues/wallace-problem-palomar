...
# Comparator/NanoDa/Landrun pins follow PalomarTemplate at
# 128a6c5ce5f48622e69927ccd639cbff401022e8. lean4export is the canonical
# v4.30.0 tag commit so that its declared toolchain matches this project.
...
checkout_exact() {
  ...
  elif [ -n "$(git -C "$destination" status --porcelain --untracked-files=normal)" ]; then
    echo "error: cached verifier checkout is dirty: $destination" >&2
    echo "remove that generated cache directory or preserve your changes elsewhere" >&2
    exit 1
  fi
