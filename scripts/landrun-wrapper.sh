...
# Comparator constructs Landrun's options but does not add that delimiter.
landrun_binary=${PALOMAR_LANDRUN_BIN:?PALOMAR_LANDRUN_BIN must name the pinned Landrun binary}
landrun_options=()

# Refuse flags that would switch off a class of Landlock restriction. Widening
# the sandbox is an explicit repository decision, not something a dependency
# bump may do unnoticed.
while [ "$#" -gt 0 ]; do
  case "$1" in
